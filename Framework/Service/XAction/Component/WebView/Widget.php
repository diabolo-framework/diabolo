...
abstract class Widget {
     ...
     * @return self
     */
    public static function setup( $options=array(), Html $hostView ) {
        $widget = new static();
    ...
    public function display() {
        ...
        if ( !file_exists($viewPath) ) {
            throw new \Exception("widget view `{$viewPath}` does not exists");
        }
    ...
    private function getViewPath() {
        if ( null === $this->widgetViewName ) {
            throw new \Exception('widget view name can not be null');
        }
